...
-- It runs AFTER the enum value has been committed in the previous migration.
...
-- Update order_items policies (if table exists)
DO $$
BEGIN
  IF EXISTS (
...
-- Update job_cards policies (if table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
...
-- Update order_timeline policies (if table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
...
-- Update payment_transactions policies (if table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
...
DO $$
BEGIN
  RAISE NOTICE '✅ All policies updated to include ''staff'' role';
END $$;
