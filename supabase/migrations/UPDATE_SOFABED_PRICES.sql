...
-- First, add strike_price_2seater_rs column if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'sofabed_database' 
    AND column_name = 'strike_price_2seater_rs'
  ) THEN
    ALTER TABLE sofabed_database 
    ADD COLUMN strike_price_2seater_rs numeric;
  END IF;
END $$;

-- Update net_price_rs and strike_price_2seater_rs based on product title
